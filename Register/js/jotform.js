...
{       // Event simulator
    ...
    * @param {type} text
     ...
     * Sends 'embeddedForm' input value
     * @returns {undefined}
     */
    isFormEmbedded: function () {
        var form = $$('.jotform-form')[0];
        var embedded = Number(window !== window.top);
        ...
        $H(prices).each(function(pair){
            ...
            if (!pair.value.recurring ) {
                ...
                if (priceText) {
                    ...
                    if (pair.value.price === "0" && !pair.value.specialPriceList) {
                    ...
                    try{
                        ...
                        var checkValues = cont.select('input,select[name*=' + input.name.replace(/\[.*$/, '') + ']').map(function(e){
                            ...
                            if (e.id.match(/input_[0-9]_quantity_[0-9]+_[0-9]+/) && e.type == 'text') {
                                ...
                                if ((cb.checked && (isNaN(e.value) || e.value == 0 || e.value.empty())) // if a product is selected and qty is not valid
